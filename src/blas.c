...
void flatten(Unum4 *x, int size, int layers, int batch, int forward)
{
    ...
    memcpy(x, swap, size*layers*batch*sizeof(Unum4));
    free(swap);
}

void weighted_sum_cpu(Unum4 *a, Unum4 *b, Unum4 *s, int n, Unum4 *c)
{
    int i;
    for(i = 0; i < n; ++i){
        c[i] = s[i]*a[i] + (1-s[i])*(b ? b[i] : (Unum4)0);
    }
}
...
void smooth_l1_cpu(int n, Unum4 *pred, Unum4 *truth, Unum4 *delta, Unum4 *error)
{
    int i;
    for(i = 0; i < n; ++i){
        Unum4 diff = truth[i] - pred[i];
        Unum4 abs_val = fabs(diff);
        if(abs_val < 1) {
            error[i] = diff * diff;
            delta[i] = diff;
        }
...
void softmax(Unum4 *input, int n, Unum4 temp, int stride, Unum4 *output)
{
    int i;
    Unum4 sum = 0;
    Unum4 largest = -FLT_MAX;
    for(i = 0; i < n; ++i){
        if(input[i*stride] > largest) largest = input[i*stride];
    }
    for(i = 0; i < n; ++i){
        Unum4 e = exp(input[i*stride]/temp - largest/temp);
        sum += e;
        output[i*stride] = e;
    }
    for(i = 0; i < n; ++i){
