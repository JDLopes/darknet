...
void train_compare(char *cfgfile, char *weightfile)
{
    ...
    while(1){
        ...
        printf("%.3f: %f, %f avg, %lf seconds, %ld images\n", (Unum4)*net.seen/N, loss, avg_loss, sec(clock()-time), *net.seen);
        free_data(train);
        if(i%100 == 0){
            char buff[256];
...
void validate_compare(char *filename, char *weightfile)
{
    ...
    for(i = 1; i <= splits; ++i){
        ...
        free_matrix(pred);
        printf("%d: Acc: %f, %lf seconds, %d images\n", i, (Unum4)correct/total, sec(clock()-time), val.X.rows);
        free_data(val);
    }
}
...
{
    int k = 32;
    Unum4 EA = 1./(1+pow(10, (b->elos[class] - a->elos[class])/400.));
    Unum4 EB = 1./(1+pow(10, (a->elos[class] - b->elos[class])/400.));
    Unum4 SA = result ? 1 : 0;
    Unum4 SB = result ? 0 : 1;
