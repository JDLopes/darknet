...
void train_cifar(char *cfgfile, char *weightfile)
{
    ...
    while(get_current_batch(net) < net->max_batches || net->max_batches == 0){
        ...
        Unum4 loss = train_network_sgd(net, train, 1);
        if(avg_loss == -1) avg_loss = loss;
        avg_loss = avg_loss*.95 + loss*.05;
        printf("%ld, %.3f: %f, %f avg, %f rate, %lf seconds, %ld images\n", get_current_batch(net), (Unum4)(*net->seen)/N, loss, avg_loss, get_current_rate(net), sec(clock()-time), *net->seen);
        if(*net->seen/N > epoch){
            epoch = *net->seen/N;
            char buff[256];
...
void test_cifar_multi(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < test.X.rows; ++i){
        image im = Unum4_to_image(32, 32, 3, test.X.vals[i]);

        Unum4 pred[10] = {0};

        Unum4 *p = network_predict(net, im.data);
        ...
        int classe = max_index(test.y.vals[i], 10);
        if(index == classe) avg_acc += 1;
        free_image(im);
        printf("%4d: %.2f%%\n", i, 100.*avg_acc/(i+1));
    }
}
...
void test_cifar(char *filename, char *weightfile)
{
    ...
    Unum4 *acc = network_accuracies(net, test, 2);
    avg_acc += acc[0];
    avg_top5 += acc[1];
...
char *labels[] = {"airplane","automobile","bird","cat","deer","dog","frog","horse","ship","truck"};
    ...
    for(i = 0; i < train.X.rows; ++i){
        image im = Unum4_to_image(32, 32, 3, train.X.vals[i]);
        int classe = max_index(train.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/train/%d_%s",i,labels[classe]);
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int classe = max_index(test.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/test/%d_%s",i,labels[classe]);
        save_image_options(im, buff, PNG, 0);
    }
}
