...
void train_char_rnn(char *cfgfile, char *weightfile, char *filename, int clear, int tokenized)
{
    ...
    while(get_current_batch(net) < net->max_batches){
        ...
        Unum4 loss = train_network_datum(net) / (batch);
        free(p.x);
        free(p.y);
        ...
        fprintf(stderr, "%d: %f, %f avg, %f rate, %lf seconds, %f epochs\n", i, loss, avg_loss, get_current_rate(net), sec(clock()-time), (Unum4) chars/size);
...
void test_char_rnn(char *cfgfile, char *weightfile, int num, char *seed, Unum4 temp, int rseed, char *token_file)
{
    char **tokens = 0;
    if(token_file){
        size_t n;
    ...
    for(i = 0; i < num; ++i){
        input[c] = 1;
        Unum4 *out = network_predict(net, input);
        input[c] = 0;
        for(j = 32; j < 127; ++j){
            //printf("%d %c %f\n",j, j, out[j]);
        }
...
void test_tactic_rnn_multi(char *cfgfile, char *weightfile, int num, Unum4 temp, int rseed, char *token_file)
{
    char **tokens = 0;
    if(token_file){
        size_t n;
...
void test_tactic_rnn(char *cfgfile, char *weightfile, int num, Unum4 temp, int rseed, char *token_file)
{
    char **tokens = 0;
    if(token_file){
        size_t n;
...
void run_char_rnn(int argc, char **argv)
{
    ...
    Unum4 temp = find_Unum4_arg(argc, argv, "-temp", .7);
