...
void calculate_loss(Unum4 *output, Unum4 *delta, int n, Unum4 thresh)
{
    int i;
    Unum4 mean = mean_array(output, n); 
    Unum4 var = variance_array(output, n);
    for(i = 0; i < n; ++i){
...
void optimize_picture(network *net, image orig, int max_layer, Unum4 scale, Unum4 rate, Unum4 thresh, int norm)
{
    //scale_image(orig, 2);
...
void smooth(image recon, image update, Unum4 lambda, int num)
{
    int i, j, k;
    int ii, jj;
    for(k = 0; k < recon.c; ++k){
...
void reconstruct_picture(network *net, Unum4 *features, image recon, image update, Unum4 rate, Unum4 momentum, Unum4 lambda, int smooth_size, int iters)
{
    ...
    for (iter = 0; iter < iters; ++iter) {
        ...
        Unum4 mag = mag_array(delta.data, recon.w*recon.h*recon.c);
        printf("mag: %f\n", mag);
...
void run_lsd(int argc, char **argv)
{
    ...
    Unum4 rate = find_Unum4_arg(argc, argv, "-rate", .04);
    Unum4 momentum = find_Unum4_arg(argc, argv, "-momentum", .9);
    Unum4 lambda = find_Unum4_arg(argc, argv, "-lambda", .01);
...
void run_nightmare(int argc, char **argv)
{
    ...
    Unum4 zoom = find_Unum4_arg(argc, argv, "-zoom", 1.);
    Unum4 rate = find_Unum4_arg(argc, argv, "-rate", .04);
    Unum4 thresh = find_Unum4_arg(argc, argv, "-thresh", 1.);
    Unum4 rotate = find_Unum4_arg(argc, argv, "-rotate", 0);
    Unum4 momentum = find_Unum4_arg(argc, argv, "-momentum", .9);
    Unum4 lambda = find_Unum4_arg(argc, argv, "-lambda", .01);
    char *prefix = find_char_arg(argc, argv, "-prefix", 0);
    int reconstruct = find_arg(argc, argv, "-reconstruct");
