...
void validate_classifier_crop(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 1; i <= splits; ++i){
        ...
        time=clock();
        Unum4 *acc = network_accuracies(net, val, topk);
        avg_acc += acc[0];
        avg_topk += acc[1];
...
void validate_classifier_10(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        free(pred);
        if(indexes[0] == classe) avg_acc += 1;
        for(j = 0; j < topk; ++j){
            if(indexes[j] == classe) avg_topk += 1;
        }
...
void validate_classifier_full(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        //cvWaitKey(0);
        Unum4 *pred = network_predict(net, resized.data);
...
void validate_classifier_single(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        //show_image(crop, "cropped");
        //cvWaitKey(0);
        Unum4 *pred = network_predict(net, crop.data);
        if(net->hierarchy) hierarchy_predictions(pred, net->outputs, net->hierarchy, 1, 1);
...
void try_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int layer_num)
{
    ...
    while(1){
        ...
        Unum4 mean[] = {0.48263312050943, 0.45230225481413, 0.40099074308742};
        Unum4 std[] = {0.22590347483426, 0.22120921437787, 0.22103996251583};
        Unum4 var[3];
        var[0] = std[0]*std[0];
        var[1] = std[1]*std[1];
        var[2] = std[2]*std[2];
...
void csv_classifier(char *datacfg, char *cfgfile, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        image r = letterbox_image(im, net->w, net->h);
        Unum4 *predictions = network_predict(net, r.data);
        if(net->hierarchy) hierarchy_predictions(predictions, net->outputs, net->hierarchy, 1, 1);
        top_k(predictions, net->outputs, top, indexes);
...
void threat_classifier(char *datacfg, char *cfgfile, char *weightfile, int cam_index, const char *filename)
{
    ...
    while(1){
        ...
        Unum4 *predictions = network_predict(net, in_s.data);
        Unum4 curr_threat = 0;
        if(1){
        ...
        for(i = 0; i < threat * h ; ++i){
            Unum4 ratio = (Unum4) i / h;
            Unum4 r = (ratio < .5) ? (2*(ratio)) : 1;
            Unum4 g = (ratio < .5) ? 1 : 1 - 2*(ratio - .5);
