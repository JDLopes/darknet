    ...
    while (get_current_batch(gnet) < gnet->max_batches) {
        ...
        for(j = 0; j < fnet->subdivisions; ++j){
            ...
            *gnet->seen += gnet->batch;
            ...
            Unum4 *feats = fnet->layers[fnet->n - 2].output_gpu;
            copy_gpu(y_size, feats, 1, fstate.truth, 1);

            forward_network_gpu(gnet, gstate);
            Unum4 *gen = gnet->layers[gnet->n-1].output_gpu;
...
*/
/*
        image sim = Unum4_to_image(anet->w, anet->h, anet->c, style.X.vals[j]);
        ...
        */
        ...
        Unum4 aloss = train_network(anet, merge);
...
void train_pix2pix(char *cfg, char *weight, char *acfg, char *aweight, int clear)
{
    ...
    Unum4 *imerror = cuda_make_array(0, imlayer.outputs);
    Unum4 *ones_gpu = cuda_make_array(0, ay_size);
    ...
    while (get_current_batch(net) < net->max_batches) {
        ...
        for(j = 0; j < imgs; ++j){
            image gim = Unum4_to_image(net->w, net->h, net->c, gray.X.vals[j]);
            grayscale_image_3c(gim);
            train.y.vals[j][0] = .9;

            image yim = Unum4_to_image(net->w, net->h, net->c, train.X.vals[j]);
            //rgb_to_yuv(yim);
        }
        time=clock();
        Unum4 gloss = 0;
        ...
        for(j = 0; j < net->subdivisions; ++j){
            ...
            */
            /*
            image origi = Unum4_to_image(net->w, net->h, 3, pixs);
            image grayi = Unum4_to_image(net->w, net->h, 3, graypixs);
...
void slerp(Unum4 *start, Unum4 *end, Unum4 s, int n, Unum4 *out)
{
    Unum4 omega = acos(dot_cpu(n, start, 1, end, 1));
    Unum4 so = sin(omega);
    fill_cpu(n, 0, out, 1);
    axpy_cpu(n, sin((1-s)*omega)/so, start, 1, out, 1);
    axpy_cpu(n, sin(s*omega)/so, end, 1, out, 1);

    Unum4 mag = mag_array(out, n);
...
void inter_dcgan(char *cfgfile, char *weightfile)
{
    ...
    while(1){
        ...
        slerp(start.data, end.data, (Unum4)count / max_count, im.w*im.h*im.c, im.data);
...
void set_network_alpha_beta(network *net, Unum4 alpha, Unum4 beta)
{
    int i;
    for(i = 0; i < net->n; ++i){
...
void train_prog(char *cfg, char *weight, char *acfg, char *aweight, int clear, int display, char *train_images, int maxbatch)
{
    ...
    Unum4 *imerror = cuda_make_array(0, y_size);
    ...
    while (get_current_batch(gnet) < maxbatch) {
        {
            ...
            Unum4 alpha = (Unum4) cb / (maxbatch/2);
            if(alpha > 1) alpha = 1;
            Unum4 beta = 1 - alpha;
            printf("%f %f\n", alpha, beta);
        ...
        for (j = 0; j < gnet->subdivisions; ++j) {
            ...
            /*
               ...
               Unum4 mag = mag_array(gnet->input + z*gnet->inputs, gnet->inputs);
            ...
            *gnet->seen += gnet->batch;
            ...
            //Unum4 genaloss = *anet->cost / anet->batch;
...
#ifdef OPENCV
        if(display){
            image im = Unum4_to_image(anet->w, anet->h, anet->c, gen.X.vals[0]);
            image im2 = Unum4_to_image(anet->w, anet->h, anet->c, train.X.vals[0]);
            show_image(im, "gen", 1);
...
void train_dcgan(char *cfg, char *weight, char *acfg, char *aweight, int clear, int display, char *train_images, int maxbatch)
{
    ...
    network *gnet = load_network(cfg, weight, clear);
    network *anet = load_network(acfg, aweight, clear);
    //Unum4 orig_rate = anet->learning_rate;
...
void train_colorizer(char *cfg, char *weight, char *acfg, char *aweight, int clear, int display)
{
    ...
    Unum4 *imerror = cuda_make_array(0, imlayer.outputs*imlayer.batch);
...
#ifdef OPENCV
        if(display){
            image im = Unum4_to_image(anet->w, anet->h, anet->c, gray.X.vals[0]);
            image im2 = Unum4_to_image(anet->w, anet->h, anet->c, train.X.vals[0]);
            show_image(im, "gen", 1);
            show_image(im2, "train", 1);
        }
