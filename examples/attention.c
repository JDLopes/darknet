...
void extend_data_truth(data *d, int n, Unum4 val)
{
    int i, j;
    for(i = 0; i < d->y.rows; ++i){
        d->y.vals[i] = realloc(d->y.vals[i], (d->y.cols+n)*sizeof(Unum4));
        for(j = 0; j < n; ++j){
...
matrix network_loss_data(network *net, data test)
{
    ...
    for(i = 0; i < test.X.rows; i += net->batch){
        for(b = 0; b < net->batch; ++b){
            ...
            memcpy(y+b*test.y.cols, test.y.vals[i+b], test.y.cols*sizeof(Unum4));
        }
        ...
        *net = orig;
        ...
        Unum4 *delta = net->layers[net->n-1].output;
        for(b = 0; b < net->batch; ++b){
            if(i+b == test.X.rows) break;
            int t = max_index(y + b*test.y.cols, 1000);
            Unum4 err = sum_array(delta + b*net->outputs, net->outputs);
            pred.vals[i+b][0] = -err;
...
void train_attention(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    Unum4 avg_cls_loss = -1;
    Unum4 avg_att_loss = -1;
...
#pragma omp parallel for
        ...
        free_data(best);
        printf("\n");
        image im = Unum4_to_image(64,64,3,resized.X.vals[0]);
        //show_image(im, "orig");
        //cvWaitKey(100);
        /*
           image im1 = Unum4_to_image(64,64,3,tiles[i].X.vals[0]);
           image im2 = Unum4_to_image(64,64,3,resized.X.vals[0]);
           show_image(im1, "tile");
           show_image(im2, "res");
         */
#ifdef GPU
        ...
        printf("%ld, %.3f: Att: %f, %f avg, Class: %f, %f avg, %f rate, %lf seconds, %ld images\n", get_current_batch(net), (Unum4)(*net->seen)/N, aloss, avg_att_loss, closs, avg_cls_loss, get_current_rate(net), what_time_is_it_now()-time, *net->seen);
        if(*net->seen/N > epoch){
            epoch = *net->seen/N;
            char buff[256];
...
void validate_attention_single(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        //cvWaitKey(0);
        Unum4 *pred = network_predict(net, rcrop.data);
        //pred[classes + 56] = 0;
        ...
        for(j = 0; j < extra; ++j){
            ...
            Unum4 *pred = network_predict(net, tile.data);
...
void validate_attention_multi(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        for(j = 0; j < nscales; ++j){
            ...
            Unum4 *p = network_predict(net, r.data);
